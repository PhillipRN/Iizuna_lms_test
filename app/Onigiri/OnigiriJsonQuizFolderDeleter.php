...
class OnigiriJsonQuizFolderDeleter
{
    public function DeleteById($teacherId, $folderId)
    {
        $Model = new OnigiriJsonQuizFolderModel();

        $folder = $Model->GetById($folderId);

        if (empty($folder)) {
            return ['error' =>  Error::ERROR_ONIGIRI_JSON_QUIZ_FOLDER_DELETE_NOT_FOUND];
        }

        if ($folder['teacher_id'] != $teacherId) {
            return ['error' =>  Error::ERROR_ONIGIRI_JSON_QUIZ_FOLDER_DELETE_NOT_PERMIT];
        }

        $childrenRecords = $Model->GetsByKeyValue('parent_folder_id', $folderId);

        if (!empty($childrenRecords)) {
            return ['error' =>  Error::ERROR_ONIGIRI_JSON_QUIZ_FOLDER_DELETE_REMAIN_CHILD];
        }
        ...
        if (!$resultJsonQuizFolder) {
            ...
            return ['error' =>  Error::ERROR_ONIGIRI_JSON_QUIZ_FOLDER_DELETE_FAILED];
        }

        $resultJsonQuiz = (new OnigiriJsonQuizModel())->MoveToRootFolder($folderId);
