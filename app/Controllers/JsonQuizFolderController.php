...
use IizunaLMS\Models\JsonQuizFolderViewModel;

class JsonQuizFolderController
{
    private $folderMap;
    ...
    private function InitializeFolderMap($schoolId)
    {
        ...
        $records = (new JsonQuizFolderViewModel())->GetsByKeyValue('school_id', $schoolId);
    ...
    public function GetFolder($folderId)
    {
        if ($folderId === 'all') return [
            'id' => 'all',
            'name' => JsonQuizFolder::ALL_FOLDER_NAME
        ];

        if ($folderId == 0) return [
            'id' => 0,
            'name' => JsonQuizFolder::ROOT_FOLDER_NAME
        ];

        return (new JsonQuizFolderModel())->GetById($folderId);
    }
    ...
    public function CreateFolderListHtml($enableRootClick=false)
    {
        $rootHtml = ($enableRootClick)
            ? '<li><a href="javascript:void(0)" data-id="0" data-parent_folder_id="0" data-teacher_id="0" class="folderNode">' . JsonQuizFolder::ROOT_FOLDER_NAME . '</a>'
            : JsonQuizFolder::ROOT_FOLDER_NAME;
    ...
    {
        $html = '<option value="0">' . JsonQuizFolder::ROOT_FOLDER_NAME . '</option>';
